    ...
    <section className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-4" aria-labelledby="source-panel-title">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--text-secondary)]">Memoria y c�digo usados para sustentar la respuesta.</p>
        </div>
      ...
      {sources.length ? (
        <div className="mt-4 grid gap-2">
          {sources.map((source) => {
            ...
            return (
              <article key={source.handle} tabIndex={0} aria-label={`${source.type === "code" ? "C�digo" : "Memoria"}: ${source.title}`} className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-3 outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)]">
      ...
      ) : <p className="mt-4 rounded-xl border border-dashed border-[var(--border-subtle)] p-4 text-sm text-[var(--text-muted)]" role="status">La respuesta a�n no tiene evidencia citada.</p>}
    </section>
  );
}
