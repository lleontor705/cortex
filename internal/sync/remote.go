...
func (s *RemoteSyncer) ensureState(ctx context.Context) error {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS remote_sync_edges (local_id INTEGER PRIMARY KEY,sync_id TEXT NOT NULL UNIQUE,from_sync_id TEXT NOT NULL DEFAULT '',to_sync_id TEXT NOT NULL DEFAULT '',relation_type TEXT NOT NULL,weight REAL NOT NULL,confidence REAL NOT NULL,source TEXT,reasoning TEXT,valid_from TEXT,valid_until TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,deleted INTEGER NOT NULL DEFAULT 0)`,
		...
		`INSERT OR IGNORE INTO remote_sync_sessions(local_id,sync_id,updated_at) SELECT id,id,COALESCE(ended_at,started_at,datetime('now')) FROM sessions WHERE id NOT IN (SELECT local_id FROM remote_sync_sessions)`,
		`INSERT OR IGNORE INTO remote_sync_sessions(local_id,sync_id,updated_at) SELECT DISTINCT session_id,session_id,datetime('now') FROM observations WHERE session_id NOT IN (SELECT local_id FROM remote_sync_sessions)`,
		`INSERT OR IGNORE INTO remote_sync_edges(local_id,sync_id,from_sync_id,to_sync_id,relation_type,weight,confidence,source,reasoning,valid_from,valid_until,created_at,updated_at) SELECT e.id,lower(hex(randomblob(16))),COALESCE(a.sync_id,lower(hex(randomblob(16)))),COALESCE(b.sync_id,lower(hex(randomblob(16)))),e.relation_type,e.weight,e.confidence,e.source,e.reasoning,e.valid_from,e.valid_until,e.created_at,e.created_at FROM edges e LEFT JOIN observations a ON a.id=e.from_obs_id LEFT JOIN observations b ON b.id=e.to_obs_id WHERE e.id NOT IN (SELECT local_id FROM remote_sync_edges) GROUP BY e.id`,
		...
		`CREATE TRIGGER IF NOT EXISTS remote_sync_edge_insert AFTER INSERT ON edges BEGIN INSERT OR IGNORE INTO remote_sync_edges(local_id,sync_id,from_sync_id,to_sync_id,relation_type,weight,confidence,source,reasoning,valid_from,valid_until,created_at,updated_at) VALUES(NEW.id,lower(hex(randomblob(16))),COALESCE((SELECT sync_id FROM observations WHERE id=NEW.from_obs_id), lower(hex(randomblob(16)))),COALESCE((SELECT sync_id FROM observations WHERE id=NEW.to_obs_id), lower(hex(randomblob(16)))),NEW.relation_type,NEW.weight,NEW.confidence,NEW.source,NEW.reasoning,NEW.valid_from,NEW.valid_until,NEW.created_at,NEW.created_at); END`,
		`CREATE TRIGGER IF NOT EXISTS remote_sync_edge_update AFTER UPDATE ON edges BEGIN UPDATE remote_sync_edges SET from_sync_id=COALESCE((SELECT sync_id FROM observations WHERE id=NEW.from_obs_id),from_sync_id),to_sync_id=COALESCE((SELECT sync_id FROM observations WHERE id=NEW.to_obs_id),to_sync_id),relation_type=NEW.relation_type,weight=NEW.weight,confidence=NEW.confidence,source=NEW.source,reasoning=NEW.reasoning,valid_from=NEW.valid_from,valid_until=NEW.valid_until,updated_at=datetime('now'),deleted=0 WHERE local_id=NEW.id; END`,
