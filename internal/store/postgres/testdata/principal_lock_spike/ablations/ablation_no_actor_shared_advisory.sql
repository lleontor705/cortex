-- PREREGISTERED ABLATION A3 (R1R7, task-54669929dd0c44adad895410d81be320):
-- single-mechanism ablation of the migration 108 read path.
-- REMOVED COMPONENT: the shared actor advisory
-- (pg_advisory_xact_lock_shared on cortex_principal_key) from BOTH readers:
-- cortex_verify_token_principal and cortex_bind_principal. Every other step
-- (lock-free resolves, unlocked re-reads, FOR SHARE revalidation, telemetry
-- usage advisory + UPDATE, HMAC provenance, tenant-context upsert) is kept
-- byte-identical to migration 108.
-- UNSAFE PRODUCTION SEMANTICS: without the shared gate the SRW
-- revalidation/revocation safety is GONE (an invalidator could commit
-- between the gateless read and the row lock). This variant exists ONLY to
-- attribute the PgBouncer-only same-principal c32 deficit to the shared
-- advisory mechanism. Applied by test onto a FRESH throwaway database after
-- the full 100..108 migration line; never applied to any certified database.
...
BEGIN
    ...
    IF v_token_id IS NULL OR v_tenant IS NULL OR v_subject IS NULL OR v_subject_active IS NOT TRUE THEN
        RETURN;
    END IF;
    -- ABLATION: the shared actor advisory gate is removed.
    SELECT t.public_id, t.tenant_id, t.name, t.token_prefix, t.expires_at, t.revoked_at,
           t.last_used_at, t.scopes, t.workspace_ids,
           COALESCE(u.public_id, s.public_id), COALESCE(u.active, s.active), s.id IS NOT NULL
      INTO v_token_id, v_tenant, v_name, v_prefix, v_expires, v_revoked, v_last_used,
