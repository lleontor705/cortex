-- PREREGISTERED ABLATION A4 (R1R7, task-54669929dd0c44adad895410d81be320):
-- single-mechanism ablation of the migration 108 read path.
-- REMOVED COMPONENT: every FOR SHARE row lock in the readers: verify's
-- FOR SHARE OF a on actor_subjects, and bind's FOR SHARE on actor_subjects
-- plus FOR SHARE OF t on api_tokens (the re-reads become plain MVCC reads).
-- KEPT: the shared actor advisory gates in both readers, both lock-free
-- resolves, the telemetry usage advisory + conditional UPDATE, the HMAC
-- provenance computation, and the tenant-context upsert.
-- UNSAFE PRODUCTION SEMANTICS: tuple-level revalidation is gone (the
-- advisory gate alone remains). This variant exists ONLY to attribute the
-- PgBouncer-only same-principal c32 deficit to the FOR SHARE mechanism
-- (including same-tuple multixact churn). Applied by test onto a FRESH
-- throwaway database after the full 100..108 migration line; never applied
-- to any certified database.
...
BEGIN
    ...
    IF v_token_id IS NULL OR v_tenant IS NULL OR v_subject IS NULL OR v_subject_active IS NOT TRUE THEN
        RETURN;
    END IF;
    -- ABLATION: the FOR SHARE revalidation becomes a plain MVCC read.
...
BEGIN
    ...
    -- ABLATION: the token re-read drops FOR SHARE OF t.
