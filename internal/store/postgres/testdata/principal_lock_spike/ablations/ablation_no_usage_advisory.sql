-- PREREGISTERED ABLATION A2 (R1R7, task-54669929dd0c44adad895410d81be320):
-- single-component ablation of the migration 108 verify read path.
-- REMOVED COMPONENT: the whole non-authoritative telemetry block (the
-- token-usage pg_try_advisory_xact_lock AND the conditional last_used_at
-- UPDATE). A1 keeps the try-lock and removes only the UPDATE, so
-- A0-A1 isolates the UPDATE and A1-A2 isolates the usage advisory itself.
-- KEPT: the shared actor advisory, both lock-free resolves, the FOR SHARE
-- revalidation, and the HMAC provenance computation.
-- UNSAFE PRODUCTION SEMANTICS: telemetry would never advance; this variant
-- exists ONLY to attribute the PgBouncer-only same-principal c32 deficit.
...
BEGIN
    ...
    END IF;
    -- ABLATION: the entire telemetry block (usage try-advisory + UPDATE)
    -- is removed; nothing between validation and provenance minting.
