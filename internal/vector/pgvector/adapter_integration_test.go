...
func TestIntegration_Pgvector_RoundTrip(t *testing.T) {
	...
	// Scoped search must exclude the sibling workspace and the legacy row.
	...
	for _, r := range resultsB {
		if r.ID != 1 {
			t.Errorf("filtered result ID = %d; PostFilter should exclude sibling and legacy points", r.ID)
		}
	}
