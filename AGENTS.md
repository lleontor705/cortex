...
- Retrieval combines FTS5 and dense vectors via RRF ($k=60$) alongside SOTA pure-Go engines: **HippoRAG** (Personalized PageRank on memory/code graphs in `internal/domain/graph`), **Adaptive-RAG** (4-tier complexity classifier in `internal/retrieval`), **LightRAG** (hierarchical community summaries), **CRAG** (confidence gating), and **ColBERT MaxSim** (late-interaction token re-ranking). Continuous AST indexing is driven by the Zero-CGO `cortex watch` file watcher daemon (`internal/domain/code/watcher.go`).
